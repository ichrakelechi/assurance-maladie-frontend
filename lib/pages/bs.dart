...
Future<List<BsData>> fetchBsData() async {
  ...
  if (response.statusCode == 200) {
    ...
    // then parse the JSON.Bslist
    var resultsJson = json.decode(response.body).cast<Map<String, dynamic>>();
    List<BsData> Bslist =
        await resultsJson.map<BsData>((json) => BsData.fromJson(json)).toList();
    return Bslist;

    // return BsData.fromJson(jsonDecode(response.body));
  } else {
    // If the server did not return a 200 OK response,
    // then throw an exception.
    throw Exception('Failed to load BsData');
  }
}
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      body: Column(mainAxisAlignment: MainAxisAlignment.start, children: [
          ...
          children: [
            Padding(
              ...
              child: ElevatedButton(
                onPressed: () {
                  Navigator.push(context,
                      MaterialPageRoute(builder: (context) => const AddBs()));
                },
        ...
        Expanded(
            child: FutureBuilder<List<BsData>>(
                future: fetchBsData(),
